...
from keras_en_parser_and_analyzer.library.dl_based_parser import line_types_ls, line_labels_ls
...
class AnnotatorGui(Frame):
    ...
    def build_line(self, table_content, line_index, line):
        ...
        # line_index_label.grid(row=line_index, column=0)
        # self.master.create_window(100, line_index*35, height=40, width=10, window=line_index_label)

        line_content_text = Text(self.master, width=100, height=1)
        line_content_text.insert(INSERT, line_content)
        # line_content_text.grid(row=line_index, column=1)
        self.master.create_window(50, line_index*35, height=40, width=500, window=line_content_text)
        ...
        # self.master.create_window(580, line_index*35, height=40, width=200, window=line_type_button)
        # line_label_button = Button(self.master, text='Label: Unknown', width=20,
        #                            command=lambda: line_label_button_click(line_index))
        # self.master.create_window(800, line_index*35, height=40, width=200, window=line_label_button)

        def line_type_dropdown_change(*args):
            table_content[line_index][1] = line_type_var.get()

        def line_label_dropdown_change(*args):
            table_content[line_index][2] = line_label_var.get()

        line_type_var = StringVar(self.master)
        line_type_var.set('content')
        line_type_var.trace('w', line_type_dropdown_change)
        line_type_label = Label(self.master, text="Type: ")
        # line_type_label.grid(row=line_index, column=2)
        self.master.create_window(350, line_index * 35, height=40, width=50, window=line_type_label)
        line_type_button = OptionMenu(self.master, line_type_var, *line_types_ls)
        # line_type_button.grid(row=line_index, column=3)
        self.master.create_window(500, line_index * 35, height=40, width=100, window=line_type_button)

        line_label_var = StringVar(self.master)
        line_label_var.set('other')
        line_label_var.trace('w', line_label_dropdown_change)
        line_label_label = Label(self.master, text="Label: ")
        # line_label_label.grid(row=line_index, column=2)
        self.master.create_window(600, line_index * 35, height=40, width=50, window=line_label_label)
        line_label_button = OptionMenu(self.master, line_label_var, *line_labels_ls)
        # line_label_button.grid(row=line_index, column=3)
        self.master.create_window(700, line_index * 35, height=40, width=100, window=line_label_button)
...
def gui_annotate(training_data_dir_path, index, file_path, file_content):
    ...
    def callback():
        ...
        for line in table_content:
            ...
            f.write(data_type + '\t' + label + '\t' + line_content)
            f.write('\n')
            f.flush()
        f.close()
...
def main():
    data_dir_path = '/media/ahmedn1/00366FA164F9C82D/Work/Paymob/ValU/CVs/cvs'  # directory to scan for any pdf files
